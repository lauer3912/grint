...
- (void) downloadRows{
   ...
   // NSString *downloadTemp = [NSString stringWithContentsOfURL:[NSURL URLWithString:@"http://www.peter-rocker.net/grint/geocodes.csv"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.row >= rows.count){
        ...
        [cell.textLabel setText:@"Enter Manually"];
        [cell.detailTextLabel setText:@"Choose if your course is not listed here"];
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!self.detailViewController) {
        self.detailViewController = [[GrintDetail3 alloc] initWithNibName:@"GrintDetail3" bundle:nil];
    }
    
    if(indexPath.row >= rows.count){
        UITextField *textField;
        UIAlertView *prompt;
        [(GrintDetail3* )self.detailViewController setCourseName:@"Manual Entry Skipped"];
        [(GrintDetail3* )self.detailViewController setCourseAddress:@"Manual Entry"];
        ...
        [(GrintDetail3* )self.detailViewController setCourseName:[[rows objectAtIndex:indexPath.row] objectAtIndex:0]];
        [(GrintDetail3* )self.detailViewController setCourseAddress:[[rows objectAtIndex:indexPath.row] objectAtIndex:2]];
    ...
    ((GrintDetail3*)self.detailViewController).username = self.username;
    
    ((GrintDetail3*)self.detailViewController).courseID = [NSNumber numberWithInt:[tableView cellForRowAtIndexPath:indexPath].tag];
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    NSLog(@"user location is: %f, %f", userLat, userLon);
