...
const CGFloat kTSAlertView_LeftMargin	= 0.0;
const CGFloat kTSAlertView_TopMargin	= 20.0;
const CGFloat kTSAlertView_BottomMargin = 0.0;
const CGFloat kTSAlertView_RowMargin	= 0.0;
...
- (NSInteger) addButtonWithTitle: (NSString *) t
{
    ...
    [b setTitleColor:[UIColor colorWithRed:57.0/255.0 green:162.0/255.0 blue:246.0/255.0 alpha:1.0] forState:UIControlStateNormal];
    [b setTitleColor:[UIColor colorWithRed:0.8 green:0.8 blue:0.8 alpha:1.0] forState:UIControlStateHighlighted];
    [b.titleLabel setFont: [UIFont systemFontOfSize: 16]];
//    b.titleLabel.textColor = [UIColor colorWithRed:57.0/255.0 green:162.0/255.0 blue:246.0/255.0 alpha:1.0];
...
- (void) onButtonPress: (id) sender
{
    ...
    if ([button.titleLabel.text rangeOfString: @"Not now"].length > 0)
        buttonIndex = 0;
    else
        buttonIndex ++;
...
- (CGSize) recalcSizeAndLayout: (BOOL) layout
{
	...
	if ( layout )
	{
		...
		if ( self.message != nil )
		{
			...
			{
				self.messageLabel.frame = CGRectMake( kTSAlertView_LeftMargin, y - 10, messageViewSize.width, messageViewSize.height );
